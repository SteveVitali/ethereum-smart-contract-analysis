...
### Creating Multiple Analyzer Instances from an Image

If you're a normal AWS user like me, Amazon won't let you create anything larger than a t2.2xlarge (8 vCPU's, 32GB memory). But we need more than that. One way around this is to select "Actions"->"Image"->"Create Image" for the t2.2xlarge version of the instance and then to create instances from that image (it may take tens of minutes or even several hours to create the image).

Now when a new instance is launched from that image, it will have all of the same EBS data attached to it and all the configuration/installation that was done on the original instance. Now we can run multiple analyzers at once for different block ranges, periodically uploading and downloading from the S3 store so that all of our instances are in sync and not double-analyzing.

To transfer the newly-generated bytecode analysis CSV to S3, run:
```
aws s3 sync ./ethereumetl/export s3://ethereum-blockchain-analysis-svitali/ethereumetl/export
```

To pull any newly-uploaded bytecode analysis CSV's from S3, run:
```
aws s3 sync s3://ethereum-blockchain-analysis-svitali .
```
